...
if ($user_status == "unverified") {
	echo unverified_modal("owner_sublet_ads.php");
...
$title = $photo = $date = $ads_for = $special_facility = $rent = $rent_type = $address = $city = $area = $details = $o_name = $o_phone = $o_email = $lift = $generator = $wifi = $file = "";
	...
	if (isset($_POST['ads_post_button'])) {
		...
		if ($err1 == "" && $err2 == "" && $err3 == "" && $err4 == "" && $err5 == "" && $err6 == "" && $err7 == "" && $err8 == "" && $err9 == "" && $err10 == "" && $err11 == "" && $err12 == "" && $err13 == "") {
			...
			if ($flag == 1) {
				...
				$sql = "INSERT INTO sublet (title, date, ads_gender, facility, rent, rent_type, address, city, area, details, owner_username, owner_user_id, owner_name, owner_phone, owner_email) VALUES ('$title','$date','$ads_for','$special_facility', '$rent','$rent_type','$address','$city', '$area','$details','$username','$user_id', '$o_name','$o_phone','$o_email');";

				if (mysqli_query($conn, $sql)) {
					$last_id = mysqli_insert_id($conn);

					$folderPath = "../images/Ad_images/owner/" . $user_id . "/sublet/";
					...
					$sql = "UPDATE sublet SET photo='$imageLinkToDB' WHERE ads_id = '$last_id';";
	...
	<section id="ad_section">
		<div class="container">
			<div class="post_ads_information">
				<form action="owner_sublet_ads.php" method="POST" enctype="multipart/form-data">
					<div>
						<h1 align="center">Post Your Sublet AD</h1>
						<table align="center">
							<tr>
								<td align="right">
									<label for="title">
							...
							<tr>
								...
								<td>
									<input type="date" name="date" id="date" min="2019-09-30" max="2050-12-31" value="<?php echo $date; ?>" style="width: 150px;" class="ads_input" required />
								</td>
							</tr>
          ...
          <option value="Uttara">Uttara</option> -->
							...
							<tr>
								...
								<td>
									<input type="tel" name="o_phone" pattern="[0-9]{11}" maxlength="11" style="width: 80%;" placeholder="e.g. 01*-********" value="<?php echo $o_phone; ?>" class="ads_input" required />
								</td>
							</tr>
							<tr>
								<td align="right">
