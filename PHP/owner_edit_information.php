...
$user_full_name = $user_email = $user_phone = $user_nid = $user_address = $user_gender = $user_dob = $user_religion = $user_password = "";
...
$sqlUserCheck = "SELECT * FROM owner WHERE username = '$username'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (isset($_POST['update_username_button'])) {
		...
		if ($err1 == "") {
			$sqlUserCheck = "SELECT username FROM owner WHERE username = '$username' and state = 'active';";
			$sqlUserCheck1 = "SELECT username FROM student WHERE username = '$username' and state = 'active';";
			...
			if ($usernameInDB == $username || $usernameInDB1 == $username) {
				...
				$sql = "UPDATE owner SET username='$username', status='unverified' WHERE user_id = '$user_id';";
	...
	if (isset($_POST['update_email_button'])) {
		...
		if ($err2 == "") {
			$sqlUserCheck = "SELECT email FROM owner WHERE email = '$user_email' and state = 'active';";
			$sqlUserCheck1 = "SELECT email FROM student WHERE email = '$user_email' and state = 'active';";
			...
			if ($user_emailInDB == $user_email || $user_emailInDB1 == $user_email) {
				...
				$sql = "UPDATE owner SET email='$user_email', status='unverified' WHERE user_id = '$user_id';";
	...
	if (isset($_POST['update_info_button'])) {
		...
		$sql = "UPDATE owner SET name='$user_full_name', religion='$user_religion', phone='$user_phone', nid='$user_nid', address='$user_address', gender='$user_gender', date_of_birth='$user_dob', status='unverified' WHERE username = '$username';";
	...
	if (isset($_POST['delete_btn'])) {
		...
		if ($err3 == "") {
			$sqlUserCheck = "SELECT password FROM owner WHERE username = '$username';";
			$result = mysqli_query($conn, $sqlUserCheck);
			while ($row = mysqli_fetch_assoc($result)) {
				$user_passwordInDB = $row['password'];
			}
			if (password_verify($user_password, $user_passwordInDB)) {
				$sql = "UPDATE owner SET state='deactive', status='unverified' WHERE username = '$username';";
...
<body>
	<?php echo after_login_owner_header("dashboard"); ?>
		...
		<div class="container">
			<div class="side_navigation">
				...
				<nav>
					<ul>
						<li id="profile_li">
							<a href="owner_profile.php" id="profile_a">Profile</a>
						</li>
						<li id="security_li">
							<a href="owner_security.php" id="security_a">Security</a>
						</li>
						<li class="current" id="information_li">
							<a href="owner_edit_information.php" id="information_a">Edit Information</a>
						</li>
					</ul>
				</nav>
			</div>
			...
			<div class="edit_information">
				...
				<fieldset>
					<details id="details3" onclick="details3()" <?php echo $information_details ?>>
						...
						<form action="owner_edit_information.php" method="POST">
							<table>
								<tr>
									<td class="label">
										<label>Name : </label>
