...
if ($user_status == "unverified") {
	echo unverified_modal("owner_flat_ads.php");
...
$err1 = $err2 = $err3 = $err4 = $err5 = $err6 = $err7 = $err8 = $err9 = $err10 = $err11 = $err12 = $err13 = $err14 = $err15 = $err16 = $err17 = "";
	...
	if (isset($_POST['ads_post_button'])) {
		...
		if (!empty($_FILES['photos']['name'][0])) {
			if (count($_FILES['photos']['name']) > 5) {
				$err17 = "You are allowed to upload maximum 5 photos.";
			} else {
				$file = $_FILES['photos'];
				// $flag = 0;
				// $file = $_FILES['photos'];
		...
		if ($err1 == "" && $err2 == "" && $err3 == "" && $err4 == "" && $err5 == "" && $err6 == "" && $err7 == "" && $err8 == "" && $err9 == "" && $err10 == "" && $err11 == "" && $err12 == "" && $err13 == "" && $err14 == "" && $err15 == "" && $err16 == "" && $err17 == "") {

			if ($flag == 1) {
				$errmsg1 = "Please keep your each file size < 500KB";
			} else {
				$sql = "INSERT INTO flat (title, date, ads_gender, size, bedroom, bathroom, corridor, facility, rent, rent_type, address, city, area, details, owner_username, owner_user_id, owner_name, owner_phone, owner_email) VALUES ('$title','$date','$ads_for','$size', '$bedroom','$bathroom','$corridor','$special_facility', '$rent','$rent_type','$address','$city', '$area','$details','$username','$user_id', '$o_name','$o_phone','$o_email');";

				if (mysqli_query($conn, $sql)) {
					$last_id = mysqli_insert_id($conn);

					$folderPath = "../images/Ad_images/owner/" . $user_id . "/flat/";
					...
					$sql = "UPDATE flat SET photo='$imageLinkToDB' WHERE ads_id = '$last_id';";
	...
	<section id="ad_section">
		<div class="container">
			<div class="post_ads_information">
				<form action="owner_flat_ads.php" method="POST" enctype="multipart/form-data">
					<div>
						<h1 align="center">Post Your Flat AD</h1>
						<table align="center">
							<tr>
								<td align="right">
									<label for="title">
