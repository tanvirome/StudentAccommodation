...
if (isset($_POST['user_id'])) {
	...
	while ($row = mysqli_fetch_assoc($result)) {
		...
		<div class="left_section_information">
			<table class="left_section_table">
				...
				<tr>
					<td align="right" class="label"><span>Institute : </span></td>
					<td align="left" class="show_data"><span><?php echo $row['institute']; ?></span></td>
				</tr>
				<tr>
					<td align="right" class="label"><span>Student ID : </span></td>
					<td align="left" class="show_data"><span><?php echo $row['student_id']; ?></span></td>
				</tr>
				<tr>
   ...
   function approveButtonClicked(event) {
    ...
    $('#load_details').load('../includes/load_admin_student_verify_details.inc.php', {
    ...
    document.getElementById("verified_btn").click();
				...
				// $('#left_section_information').load('../includes/load_admin_student_verify_details.inc.php', {
    //  'user_id': buttonValue
    // });
   }
   ...
   function leftDeleteButtonClicked(event) {
	...
	<?php
		...
		if ($button_value == "All") {
			...
			$sql = "SELECT * FROM student WHERE status = 'verified' AND state = 'active';";
			$result = mysqli_query($conn, $sql);
		} else if($button_value == "Unverified"){
			$sql = "SELECT * FROM student WHERE status = 'unverified' AND state = 'active';";
			$result = mysqli_query($conn, $sql);
		} else if($button_value == "Deleted"){
			$sql = "SELECT * FROM student WHERE state = 'deactive';";
			$result = mysqli_query($conn, $sql);
		}
			?>
		<table class="right_section_table" id="right_section_table">
			<thead>
				<tr>
					<th class="user_id">Student ID</th>
   ...
   var rowClicked = 0;
	...
	<?php
		...
		if ($button_value == "Approve") {
			$sql = "UPDATE student SET status='verified', state ='active' WHERE user_id = $user_id;";
			mysqli_query($conn, $sql);
		} else if ($button_value == "Disapprove") {
			$sql = "UPDATE student SET status='unverified' WHERE user_id = $user_id;";
			mysqli_query($conn, $sql);
		} else if ($button_value == "Delete") {
			$sql = "UPDATE student SET state = 'deactive' WHERE user_id = $user_id;";
			mysqli_query($conn, $sql);
		}
